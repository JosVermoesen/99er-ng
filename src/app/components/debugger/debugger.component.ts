...
export class DebuggerComponent implements OnInit, OnChanges, OnDestroy {
        ...
        if (this.visible && this.ti994A) {
            ...
            switch (this.memoryViewType) {
                ...
                case MemoryViewType.SPLIT:
                    memoryView = this.getDisassemblyView(
                        this.ti994A.isGPUActive()
                            ? MemoryType.VDP
                            : MemoryType.CPU
                    );
                    const memoryView2 = this.getHexView(this.memoryType, true);
                    this.memoryString2 = memoryView2.lines
                        .map((l) => l.text)
                        .join("\n");
            ...
            if (force || this.memoryViewType !== MemoryViewType.LIST) {
                this.memoryString = memoryView.lines
                    .map((l) => l.text)
                    .join("\n");
            }
    ...
    scrollToAnchorLine(memoryView: MemoryView, id: string) {
        ...
        if (memoryView.anchorLine != null && memoryView.lines.length > 0) {
            ...
            this.scrollTimeoutHandles[id] = setTimeout(function () {
                const lineHeight =
                    $memory.prop("scrollHeight") / memoryView.lines.length;
    ...
    getDisassemblyView(memoryType: MemoryType): MemoryView {
        ...
        if (this.ti994A.isRunning()) {
            ...
            memoryView = this.disassemblerService.disassemble(
                pc,
                null,
                32,
                pc,
            ...
            // Stopped
            const viewAddress =
                this.memoryViewType === MemoryViewType.SPLIT
                    ? pc
                    : this.getViewAddress(pc);
            if (memoryType === MemoryType.CPU) {
                // CPU
                this.disassemblerService.setMemory(this.ti994A.getMemory());
                memoryView = this.disassemblerService.disassemble(
                ...
                memoryView.lines.forEach((memoryLine) => {
                    ...
                    if (cycles) {
                        memoryLine.text =
                            Util.padr(memoryLine.text + " ", " ", 38) + cycles;
                    }
                });
            } else {
                // VDP
                ...
                memoryView = this.disassemblerService.disassemble(
                    0,
                    this.ti994A.getVDP().getGPU() ? 0x4800 : 0x4000,
                    null,
                    viewAddress,
                    breakpointAddress
                );
    ...
    getHexView(memoryType: MemoryType, narrow: boolean): MemoryView {
        ...
        if (this.ti994A.isRunning()) {
            ...
            if (memoryType === MemoryType.CPU) {
                ...
                memoryView = this.ti994A
                    .getMemory()
                    .hexView(viewAddress, 512, width, viewAddress);
            } else {
                // VDP
            ...
            if (memoryType === MemoryType.CPU) {
                ...
                memoryView = this.ti994A
                    .getMemory()
                    .hexView(viewAddress % width, 0x10000, width, viewAddress);
            } else {
                // VDP
                memoryView = this.ti994A
                    .getVDP()
    ...
    getListView(memoryType: MemoryType): MemoryView {
        if (this.listView && memoryType === MemoryType.CPU) {
            if (this.lastAnchorLine != null) {
                this.lastAnchorLine.text =
                    " " + this.lastAnchorLine.text.substring(1);
                this.lastAnchorLine = null;
            }
            if (this.lastBreakpointLine != null) {
                this.lastBreakpointLine.text =
                    this.lastBreakpointLine.text.charAt(0) +
                    " " +
                    this.lastBreakpointLine.text.substring(2);
            ...
            if (breakpointLine) {
                breakpointLine.text =
                    breakpointLine.text.charAt(0) +
                    "\u25cf" +
                    breakpointLine.text.substring(2);
                this.lastBreakpointLine = breakpointLine;
            }
            this.listView.breakpointLine =
                this.lastBreakpointLine?.index || null;
    ...
    onMemoryViewClicked(event: MouseEvent) {
        if (
            this.memoryViewType !== MemoryViewType.HEX &&
            !this.ti994A.isRunning() &&
            this.memoryView &&
            event.offsetX < 64
        ) {
            const $memory = $(this.element.nativeElement).find(
                "#" + this.getAssemblyViewId()
            );
            const lineHeight =
                $memory.prop("scrollHeight") / this.memoryView.lines.length;
            const lineNo = Math.floor(
                ($memory.prop("scrollTop") + event.offsetY) / lineHeight
            );
    ...
    getAssemblyViewId() {
        return this.memoryViewType !== MemoryViewType.SPLIT
            ? "memory"
            : "memory1";
    }
