...
export class DiskComponent implements OnInit, OnDestroy {
        ...
        switch (event.type) {
            ...
            case ConsoleEventType.DISK_REMOVED:
                ...
                if (this.deletingDisk) {
                    this.commandDispatcherService.deleteDisk(
                        this.diskImages[this.diskImageIndex]
                    );
    ...
    insertDisk() {
        ...
        if (index >= 0) {
            this.commandDispatcherService.insertDisk(
                this.driveIndex,
                this.diskImages[index]
            );
        } else {
    ...
    deleteFiles() {
        this.commandDispatcherService.deleteDiskFiles(
            this.diskImages[this.diskImageIndex],
    ...
    saveDisk() {
        this.commandDispatcherService.saveDisk(
            this.diskImages[this.diskImageIndex]
        );
    }
    ...
    private updateAllDiskImageDrives() {
        for (let i = 0; i < this.diskImages.length; i++) {
            this.diskImageDrives[i] = this.updateDiskImageDrives(
                this.diskImages[i]
            );
        }
    }
