...
export class MainComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.log.info(
            "Welcome to " +
                Js99erComponent.TITLE +
                " version " +
                Js99erComponent.VERSION
        );
        this.log.info("--------------------------------");
    }
    ...
    configure() {
        ...
        if (config) {
            this.sidePanelVisible =
                config.sidePanelVisible !== undefined
                    ? config.sidePanelVisible
                    : true;
            this.toolbarVisible =
                config.toolbarVisible !== undefined
        ...
        switch (event.type) {
            case ConsoleEventType.READY:
                ...
                this.audioService.init(
                    this.settingsService.isSoundEnabled(),
                    this.ti994A.getPSG(),
                    this.ti994A.getSpeech(),
                    this.ti994A.getTape()
                );
                if (this.cartURL) {
    ...
    loadCartridge(cartName: string) {
        ...
        this.moreSoftwareService
            .getByName(cartName.replace(/_/g, " "))
            .subscribe({
                next: (cart: Software) => {
        ...
        database
            ...
            .pipe(
                map((diskImages: DiskImage[]) => {
                    ...
                    const diskDrives = that.ti994A.getDiskDrives();
                    return that.diskService.restoreDiskDrives(
                        diskDrives,
                        diskImages
                    );
                })
            ...
            .subscribe({
                next: (state: any) => {
                    ...
                    if (f18AEnabled && !that.settingsService.isF18AEnabled()) {
                        that.log.error(
                            "Please enable F18A before restoring the state"
                        );
                        return;
                    } else if (
                        !f18AEnabled &&
                        that.settingsService.isF18AEnabled()
                    ) {
                        that.log.error(
                            "Please disable F18A before restoring the state"
                        );
                        return;
                    }
                    ...
                    settings.setSoundEnabled(
                        that.settingsService.isSoundEnabled()
                    );
                    settings.setSpeechEnabled(state.speech.enabled);
                    settings.set32KRAMEnabled(state.memory.enable32KRAM);
                    settings.setF18AEnabled(
                        that.settingsService.isF18AEnabled()
                    );
                    settings.setPCKeyboardEnabled(
                        state.keyboard.pcKeyboardEnabled
                    );
                    settings.setMapArrowKeysEnabled(
                        state.keyboard.mapArrowKeysToFctnSDEX
                    );
                    settings.setGoogleDriveEnabled(
                        that.settingsService.isGoogleDriveEnabled()
                    );
                    settings.setSAMSEnabled(state.memory.enableSAMS);
                    settings.setGRAMEnabled(state.memory.enableGRAM);
                    settings.setPixelatedEnabled(
                        that.settingsService.isPixelatedEnabled()
                    );
                    ...
                    if (state.tape.recordPressed) {
                        ...
                        that.eventDispatcherService.tapeStopped(
                            tape.isPlayEnabled(),
                            tape.isRewindEnabled()
                        );
                    }

                    that.commandDispatcherService.setBreakpointAddress(
                        state.cpu.breakpoint
                    );
