...
export class DiskFile implements State {
    ...
    getDataTypeLabel(): string {
        return this.getFileType() === FileType.DATA
            ? Disk.DATA_TYPE_LABELS[this.dataType]
            : "-";
    }
    ...
    getRecordTypeLabel(): string {
        return this.getFileType() === FileType.DATA
            ? Disk.RECORD_TYPE_LABELS[this.recordType]
            : "-";
    }
    ...
    getAccessTypeLabel(): string {
        return this.getFileType() === FileType.DATA
            ? Disk.ACCESS_TYPE_LABELS[this.accessType]
            : "-";
    }
    ...
    getSectorCount(): number {
        ...
        if (this.getFileType() === FileType.DATA) {
            if (this.getRecordType() === RecordType.FIXED) {
                ...
                sectors =
                    Math.floor(this.records.length / recsPerSector) +
                    (this.records.length % recsPerSector === 0 ? 0 : 1);
            } else {
                sectors = 1;
            ...
            sectors =
                Math.floor(this.program.length / 256) +
                (this.program.length % 256 === 0 ? 0 : 1);
        }
        return sectors;
    }
    ...
    getEOFOffset(): number {
        ...
        if (this.getFileType() === FileType.DATA) {
            if (this.getRecordType() === RecordType.FIXED) {
                const recsPerSector = Math.floor(256 / this.recordLength);
                eofOffset =
                    (this.getRecordCount() % recsPerSector) * this.recordLength;
            } else {
                let sectorBytesLeft = 256;
                for (let i = 0; i < this.records.length; i++) {
                    const recordSize = this.records[i].getData().length + 1;
