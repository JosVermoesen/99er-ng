...
export class CommandDispatcherService {
    ...
    private commandObservable: Observable<Command> =
    ...
    loadDisk(driveIndex: number, files: FileList) {
        this.commandSubject.next(
            new Command(CommandType.LOAD_DISK, {
    ...
    loadSoftware(software: Software) {
        this.commandSubject.next(
            new Command(CommandType.LOAD_SOFTWARE, { software: software })
        );
    }

    unloadSoftware() {
        this.commandSubject.next(
            new Command(CommandType.UNLOAD_SOFTWARE, null)
        );
    }

    changeSetting(setting: Setting, value: boolean | string) {
        this.commandSubject.next(
            new Command(CommandType.CHANGE_SETTING, {
                setting: setting,
                value: value,
            })
        );
    ...
    screenshot() {
        this.commandSubject.next(
            new Command(CommandType.TAKE_SCREENSHOT, null)
        );
    }
    ...
    setBreakpointAddress(addr: number) {
        this.commandSubject.next(
            new Command(CommandType.SET_BREAKPOINT_ADDRESS, addr)
        );
    }
    ...
    insertDisk(driveIndex: number, diskImage: DiskImage) {
        this.commandSubject.next(
            new Command(CommandType.INSERT_DISK, {
                driveIndex: driveIndex,
                diskImage: diskImage,
            })
        );
    ...
    removeDisk(driveIndex: number) {
        this.commandSubject.next(
            new Command(CommandType.REMOVE_DISK, driveIndex)
        );
    }

    deleteDisk(diskImage: DiskImage) {
        this.commandSubject.next(
            new Command(CommandType.DELETE_DISK, diskImage)
        );
    }

    deleteDiskFiles(diskImage: DiskImage, diskfiles: DiskFile[]) {
        this.commandSubject.next(
            new Command(CommandType.DELETE_DISK_FILES, {
                diskImage: diskImage,
                diskFiles: diskfiles,
            })
        );
    ...
    startRecording() {
        this.commandSubject.next(
            new Command(CommandType.START_RECORDING, null)
        );
    }
    ...
    toggleSidePanel(visible: boolean) {
        this.commandSubject.next(
            new Command(CommandType.TOGGLE_SIDE_PANEL, visible)
        );
    }

    requestPointerLock() {
        this.commandSubject.next(
            new Command(CommandType.REQUEST_POINTER_LOCK, null)
        );
    }
}
