...
export class EventDispatcherService {
    ...
    private eventObservable: Observable<ConsoleEvent> =
    ...
    diskAdded(diskImage: DiskImage) {
        this.sendAsyncEvent(ConsoleEventType.DISK_ADDED, {
            diskImage: diskImage,
        });
    }

    diskInserted(diskDrive: DiskDrive, diskImage: DiskImage) {
        this.sendAsyncEvent(ConsoleEventType.DISK_INSERTED, {
            diskDrive: diskDrive,
            diskImage: diskImage,
        });
    }
    ...
    diskRemoved(diskDrive: DiskDrive, diskImage: DiskImage) {
        this.sendAsyncEvent(ConsoleEventType.DISK_REMOVED, {
            diskDrive: diskDrive,
            diskImage: diskImage,
        });
    }
    ...
    diskDeleted(diskImage: DiskImage) {
        this.sendAsyncEvent(ConsoleEventType.DISK_DELETED, {
            diskImage: diskImage,
        });
    }
    ...
    diskDriveChanged(diskDriveIndex: number) {
        this.sendAsyncEvent(
            ConsoleEventType.DISK_DRIVE_CHANGED,
            diskDriveIndex
        );
    }
    ...
    tapeStopped(playEnabled: boolean, rewindEnabled: boolean) {
        this.sendAsyncEvent(ConsoleEventType.TAPE_STOPPED, {
